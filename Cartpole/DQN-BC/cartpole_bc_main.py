...
def serial_pipeline_bc(
    ...
    evaluator_env.enable_save_replay(replay_path='cartpole_bc_seed0/video')
    # Random seed
...
if __name__ == '__main__':
    ...
    expert_policy = serial_pipeline([cartpole_dqn_config, cartpole_dqn_create_config], seed=0)

    collect_count = 10000  # number of transitions to collect
    expert_data_path = 'expert_data_dqn_bc.pkl'  # data path to be saved
    state_dict = expert_policy.collect_mode.state_dict()
    collect_config = [deepcopy(cartpole_dqn_config), deepcopy(cartpole_dqn_create_config)]
    collect_config[0].exp_name = 'cartpole_dqn_bc_collect'
    collect_demo_data(
        collect_config, seed=0, state_dict=state_dict, expert_data_path=expert_data_path, collect_count=collect_count
    )

    from cartpole_bc_config import main_config, create_config
